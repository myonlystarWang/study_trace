...
1. **APScheduler**：显式 `timezone=Asia/Shanghai`；可配置时段（默认 20:10 / 21:10 / 21:50）；生产 `--no-reload`，dev 模式文件锁防重复
2. **分时分级日报与催办策略（幂等）**：
...
- [ ] 晚报总结：21:50 时段实测当全部作业完成时，成功送达「🎉 今日全部完成」总结晚报；中途催办时段自动跳过免打扰
...
- [ ] 时区：服务器时间改为 UTC 后，仍在北京时间 20:10 触发

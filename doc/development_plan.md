...
- [ ] Streak 计算：连续 3 天 → 3；中断 1 天 → 归 1；同一天多次打勾不重复计数；跨月正确
- [ ] 艾宾浩斯：新建错题 `next_review_date = today+1`；标记掌握 → +3 → +7 → +15；标记未掌握 → 回退 `today+1`；已掌握出队
- [ ] 备份往返：导出 zip → 清空 db 与 uploads → 导入 → 记录数、图片 sha256 100% 一致
- [ ] 一键转错题：作业条目转错题后，`subject_id`、`source_reference` 正确继承
- [ ] 图片压缩：5MB iPhone 照片 → 原图 ≤400KB、缩略图 ≤40KB，且 EXIF 方向正确（竖拍不躺倒）
...
**目标**：到点主动催办，孩子能用番茄钟专注；多渠道并存轮询，哪个好用用哪个。
...
2. **幂等**：`NotificationLog(id, date, slot, channel, sent_at)`，同 `(date, slot, channel)` 只发一次；**当天作业已全部完成则跳过催办**
3. **多渠道通知分发器（轮询与并存）**：
...
5. **家长设置视图**：提醒时段、通道多选开关、各通道独立配置与「一键测试」按钮

### 验收标准（DoD）

- [ ] 定时精度：设定时间 ±30s 内触发
- [ ] 幂等：手动重复触发同一 `(date, slot)`，只发一次
- [ ] 跳过逻辑：当天作业全部完成后，到点**不**发送催办

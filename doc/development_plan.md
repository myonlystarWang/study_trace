...
⚠️ **iOS Web Push 依赖 HTTPS**，但 **群机器人 Webhook 与 iOS Bark 推送无需 HTTPS**。故 M3 可在本地局域网无缝完整测试 Webhook 与 Bark，PWA Web Push 在具备 HTTPS 时即可同步生效。

### 任务

1. **APScheduler**：显式 `timezone=Asia/Shanghai`；可配置时段（默认 17:30 / 19:30 / 21:00）；生产 `--no-reload`，dev 模式文件锁防重复
2. **分时分级日报与催办策略（幂等）**：
   - `NotificationLog(id, date, slot, channel, sent_at)`，同 `(date, slot, channel)` 只发一次
   - **中途催办时段（默认 17:30 / 19:30）**：若当天作业全部完成，自动静默跳过免打扰；若有未完成项，发送催办提醒（附待办清单）
   - **晚间日报时段（默认 21:00）**：若当天作业已全部完成，依然推送「🎉 今日满卡」表扬晚报（含连续打卡天数、各科完成用时）；若仍有未完成，则发出最终汇总与预警
   - 结构化富文本模板：包含日期、打卡天数、完成度进度条、各科条目状态、艾宾浩斯复习情况
3. **多渠道通知分发器（轮询与并存）**：
   - **群机器人 Webhook**：企微 / 钉钉 / 飞书 Webhook，设置页可填 URL + 测试按钮（100% 免费、零门槛）
   - **iOS Bark 推送**：全家 iPhone 生态免 HTTPS 极简推送神器，填 Key 即可秒弹横幅通知
   - **iOS Web Push**：`vite-plugin-pwa`（injectManifest）+ `sw.js`；VAPID 密钥对；`PushSubscriptions` 表（字段按问题 #19 修正）
4. **番茄钟**：25 分钟倒计时 + 完成声效 + 后台计时（页面切走不中断）
5. **家长设置视图**：提醒时段、通道多选开关、各通道独立配置与「一键测试」及「立即发送今日汇总」按钮
6. **月度作业打卡日历（轻量月历浮窗）**：
   - 后端增加 `GET /api/homework/calendar?month=YYYY-MM`，聚合返回当月每日的 `total`、`completed` 与红/黄/绿状态（绿色=100%，黄色=部分，红色=0%，灰色=无作业）
   - 前端 `HomeworkView.vue` 点击顶部日期或日历图标 📅，弹出月历浮窗，在日期下方标出打卡状态圆点；点击任意日期直接切换加载该日作业清单

### 验收标准（DoD）

- [ ] 定时精度：设定时间 ±30s 内触发
- [ ] 幂等：手动重复触发同一 `(date, slot)`，只发一次
- [ ] 晚报总结：21:00 时段实测当全部作业完成时，成功送达「🎉 今日全部完成」总结晚报；中途催办时段自动跳过免打扰
- [ ] 催办提醒：当天有作业未完成时，各渠道收到带待办作业清单的结构化汇总
- [ ] 月度日历 API：`GET /api/homework/calendar?month=2026-09` 响应 ≤ 50ms，正确按红黄绿输出每天汇总
- [ ] 作业页月历交互：点击顶部日期/日历图标弹出月历，红黄绿圆点准确，点击任意日期即切换加载该天作业
- [ ] 群机器人：设置页点"测试" → 群里 5s 内收到消息；Webhook 填错时给出明确错误
- [ ] Bark 推送：设置页填入 Bark Key 点测试 → iPhone 锁屏秒弹系统通知
- [ ] 多渠道并行：同时勾选 Webhook、Bark 等渠道时，并行/轮询触发，互不阻塞
- [ ] 时区：服务器时间改为 UTC 后，仍在北京时间 19:30 触发
- [ ] iOS 推送：HTTPS 下 iPhone 添加到主屏幕 → 授权 → 服务端触发 → 锁屏状态下收到横幅
- [ ] dev `--reload` 模式下不重复发送（文件锁生效）
- [ ] 番茄钟：切到后台 25 分钟后回到前台，剩余时间正确，提示音响起

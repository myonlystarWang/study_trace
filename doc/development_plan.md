...
- **优先级次序**：**微信公众平台接口测试号 (Sandbox 官方直推)** > **iOS Bark (iPhone 极简多机群发)** > **群机器人 Webhook (企微/钉钉/飞书)** > **备用渠道 (WxPusher / Server酱 / PushPlus)**
- **验证解耦策略**：微信官方测试号、iOS Bark 与群机器人**均无需依赖公网复杂基建**，在 M3 本地局域网与公网环境均可 100% 端到端真机验收闭环；iOS Web Push 依赖公网 HTTPS 与正式域名，代码与数据表在 M3 交付，真机端到端验收标记为「待 M6 验收」，不阻塞 M3 交付。
...
3. **多渠道通知分发器（轮询与并存）**：
   - **微信官方测试号（首选直推）**：接入腾讯微信公众平台接口测试号（Sandbox），0 元免费、10 万次/天额度、原生模板消息弹窗与提示音；支持配置卡片直跳链接 `https://study.raddishlab.tech`；前端支持家庭成员多 OpenID 结构化卡片管理（爸爸、妈妈单独测试与并发广播）
   - **iOS Bark 推送**：全家 iPhone 生态免 HTTPS 极简推送神器，支持逗号/换行录入多个 Key，全家设备并发群发
   - **群机器人 Webhook**：企微 / 钉钉 / 飞书 Webhook，设置页可填 URL + 测试按钮（100% 免费、零门槛）
   - **向后兼容备用渠道**：支持 WxPusher / Server酱 / PushPlus 作为辅助备选
...
- [ ] **微信官方测试号**（自动化）：微信官方 access_token 缓存自愈、多 OpenID 解析、模板消息拼装合法，卡片跳转 URL 正确
- [ ] **Bark 推送**（自动化）：设置页填入 Bark Key（支持多 Key 并发群发）点测试 → 请求正确拼装分组、标题与正文
